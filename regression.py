...
# We are guessing the estimated salary and it is a continuous value ((not categorical classes like “Yes/No” or “High/Low”))and hence it would be a regression problem statement
...
data.drop(["RowNumber","Surname","CustomerId"], axis=1, inplace=True) # axis = 0 will drop rows and axis = 1 drops columns, inplace = true will give new dataframe
print(data.head())
...
X= data.drop(['EstimatedSalary'], axis=1) # independent
y = data['EstimatedSalary'] # dependent
print(X.head())
print(y.head())
...
model = Sequential([
    ...
    Dense(1)# output layer with 1 neuron and no activation means linear activation which is needed for regression
])
...
model.compile(loss='mean_absolute_error', optimizer= optimizer, metrics=['mae']) # for regression we use these
...
print(history)
model.save('regressionmodel.h5') #h5 is compatible with Keras
...
# tensorboard --logdir=regression_logs/fit run this command in terminal
